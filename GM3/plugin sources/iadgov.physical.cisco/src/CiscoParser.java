...
public class CiscoParser {
    ...
    protected static class Interface {
        ...
        public boolean trunk = false;   //Switch-switch connection.  Sometimes the config lies.
        public boolean gateway = false; //Router port.
        public HardwareAddress address = null; // Probably supplied as a MAC in the form xxxx.xxxx.xxxx
        ...
        private final Set<HardwareAddress> connections;
        ...
        public void addConnection(final HardwareAddress target) {
            this.connections.add(target);
        }
        public Collection<HardwareAddress> getConnections() {
        ...
        for(final String nameGroup : dev.interfaces.keySet().stream().map(item -> item.getName()).distinct().sorted().collect(Collectors.toList())) {
            ...
            dev.interfaces.entrySet().stream()
                    ...
                    .forEach(i -> {
                        ...
                        port.getConnectedAddresses().addAll(i.getConnections());
