...
import iadgov.fingerprint.Reference;
...
public class PayloadFunctions {
    ...
    public static void anchorFunction(PayloadAccessor payload, CursorImpl cursor, Cursor cursorType, Position position, int offset) {
        if (position != null) {
            switch (position) {
    ...
    public static void byteJumpFunction(PayloadAccessor payload, CursorImpl cursor, int offset, boolean relative, int bytes, Endian endian, String calc) {
        int location = 0;
        if (payload.size() > offset) {
            try {
                ...
                Logger.log(Logger.Severity.PEDANTIC_DEVELOPER_SPAM, "Buffer Underflow attempting byte jump");
            }
            ...
            if (relative) {
                if (cursor.getMain() + location < payload.size()) {
                    cursor.forward(location);
                } else {
                    cursor.setMain(payload.size() - 1);
                }
            } else {
                if (location < payload.size()) {
    ...
    public static boolean byteTestFunction(PayloadAccessor payload, CursorImpl cursor, Test operator, int testVal, boolean relative, int offset, int postOffset, int bytes, Endian endian) {
        ...
        try {
            ...
            Logger.log(Logger.Severity.PEDANTIC_DEVELOPER_SPAM, "Buffer Underflow attempting byte test");
        }
        if (postOffset != 0)
    ...
    public static Map.Entry<String, String> extractFunction(PayloadAccessor payload, CursorImpl cursor, String name, String fromString, String toString, int maxLength, Endian endian, ContentType convert, Lookup lookup) {
        ...
        byte[] ext = payload.extract(from, to, maxLength, endian == Endian.BIG);
    ...
    public static boolean matchFunction(PayloadAccessor payload, CursorImpl cursor, int depth, int offset, boolean relative,
                                        boolean noCase, Endian endian, String patternString, byte[] content, boolean move, Charset charset) {
        ...
        if (patternString != null) {
            try {
                String string = new String(payload.getByteArray(offset, length, endian == Endian.BIG), charset);
                Pattern pattern;
                if (noCase) {
                ...
                Logger.log(Logger.Severity.PEDANTIC_DEVELOPER_SPAM, "Buffer Underflow attempting string match");
            }
