...
public class PayloadAccessor {
     ...
     */
    public byte[] getByteArray(int index, int length, boolean bigEndian) throws BufferUnderflowException{
        if (this.payloadStart + index + length > this.payload.limit()) {
            throw new BufferUnderflowException();
        }
        byte[] ret = new byte[length];
        this.getByteArray(index, ret, 0, length, bigEndian);
    ...
    public BigInteger getInteger(int index, int length, boolean bigEndian) throws BufferUnderflowException{
        BigInteger ret;
        byte[] bytes = this.getByteArray(index, length, bigEndian);
    ...
    public byte[] extract(int from, int to, int length, boolean bigEndian) {
        byte[] ret = new byte[0];
        ...
        if (from >= 0 && from < payload.limit() - payloadStart && to >= 0 && to < payload.limit() - payloadStart) {
            ...
            ret = getByteArray(start, end - start, bigEndian);
        }
