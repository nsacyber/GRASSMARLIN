...
public class ExtractDialog extends Dialog<ExtractDialog> {
    ...
    private class ExtractRow extends HBox{
        ...
        public ExtractRow(Extract extract, VBox parent) {
            ...
            Tooltip nameTip = new Tooltip("The name of the property");
            Tooltip.install(nameLabel, nameTip);
            Tooltip.install(nameField, nameTip);

            VBox postBox = new VBox(2);
            HBox convertBox = new HBox(2);
            HBox lookupBox = new HBox(2);
            Label postLabel = new Label("Post:");
            Tooltip postTip = new Tooltip("Post processing of extracted value");
            Tooltip.install(postLabel, postTip);
            ...
            Tooltip convertTip = new Tooltip("Convert the extracted bytes");
            Tooltip.install(convertLabel, convertTip);
            Tooltip.install(convertChoice, convertTip);
            ...
            Tooltip lookupTip = new Tooltip("Convert extracted bytes to an integer and lookup value in selected data source");
            Tooltip.install(lookupLabel, lookupTip);
            Tooltip.install(lookupChoice, lookupTip);
            ...
            Tooltip fromTip = new Tooltip("The position to start extracting from");
            Tooltip.install(fromLabel, fromTip);
            Tooltip.install(fromPositionBox, fromTip);
            Tooltip.install(fromPositionField, fromTip);

            //set the from options appropriately from the passed in extract object
            try {
                //We just need to verify it can parse.
            ...
            Tooltip toTip = new Tooltip("The stop position to extract to");
            Tooltip.install(toLabel, toTip);
            Tooltip.install(toPositionBox, toTip);
            Tooltip.install(toPositionField, toTip);

            //set the to options appropriately from the passed in extract object
            try {
                //We just need to verity it can parse.
                //noinspection ResultOfMethodCallIgnored
            ...
            Tooltip maxLengthTip = new Tooltip("The maximum number of bytes to extract");
            Tooltip.install(maxLengthLabel, maxLengthTip);
            Tooltip.install(maxLengthField, maxLengthTip);
            ...
            Tooltip endianTip = new Tooltip("The endianess of the extracted data, either big(most significant byte first) or little(most significant byte last)");
            ...
            removeButton.setTooltip(new Tooltip("Delete this row"));
