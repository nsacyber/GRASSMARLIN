...
public abstract class OpRow implements ParentBox {
    ...
    private ComboBox<PayloadItem.OpType> opBox;
    ...
    public void insert(ParentBox parent, int index) {
        ...
        opBox = new ComboBox<>(parent.getAvailableOps());
        ...
        removeButton.setOnAction(event -> {
            parent.getChildrenBox().getChildren().remove(row);
            parent.removeChild(this);
            update();
        });
    ...
    protected void updateOps() {
        if (this.opBox != null) {
            this.opBox.setItems(parent.getAvailableOps());
        }
    }

    public abstract HBox getInput();

    public abstract Serializable getOperation();
