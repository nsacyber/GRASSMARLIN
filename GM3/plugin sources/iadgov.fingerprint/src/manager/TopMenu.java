...
public class TopMenu extends MenuBar{
    ...
    private void createMenu() {
        ...
        saveItem.disableProperty().bind(this.gui.isSelectedSavable().not());
            ...
            for (File toLoad : toLoadList) {
                try {
                    if (this.gui.getDocument().alreadyLoaded(null, toLoad.toPath())) {
                        ...
                        FingerprintState state = this.gui.getDocument().load(toLoad.toPath());
                        if (state != null) {
                            this.gui.openFingerprint(state);
                        }
                    }
