...
public class DsizeWithinFilter implements Filter<Fingerprint.Filter.DsizeWithin> {
    ObjectFactory factory;
    Fingerprint.Filter.DsizeWithin value;
    SimpleObjectProperty<JAXBElement<Fingerprint.Filter.DsizeWithin>> element;

    public DsizeWithinFilter(JAXBElement<Fingerprint.Filter.DsizeWithin> value) {
        factory = new ObjectFactory();
        element = new SimpleObjectProperty<>();
        if (null == value) {
            this.value = new Fingerprint.Filter.DsizeWithin();
            this.value.setMin(new BigInteger("0"));
            this.value.setMax(new BigInteger("0"));
            element.setValue(factory.createFingerprintFilterDsizeWithin(this.value));
        } else {
    ...
    public HBox getInput() {
        ...
        Tooltip minTip = new Tooltip("Minimum number of bytes in packet data");
        Tooltip maxTip = new Tooltip("Maximum number of bytes in packet data");
        ...
        maxField.textProperty().addListener((observable, oldValue, newValue) -> {
            if (!oldValue.equals(newValue)) {
                ...
                try {
                    ...
                    value.setMax(newMax);
                    element.setValue(factory.createFingerprintFilterDsizeWithin(value));
    ...
    public FilterType getType() {
        return FilterType.DSIZEWITHIN;
    }

    @Override
    public SimpleObjectProperty<JAXBElement<Fingerprint.Filter.DsizeWithin>> elementProperty() {
        return element;
    }
}
