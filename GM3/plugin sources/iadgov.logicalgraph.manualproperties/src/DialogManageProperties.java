    ...
    public DialogManageProperties(final GraphLogicalVertex vertex) {
        ...
        this.setOnCloseRequest(event -> {
            ...
            for(final String key : map.keySet()) {
                //TODO: Support an option to set directly or to pass through the pipeline
                vertex.setProperties(Plugin.NAME, key, map.get(key).stream().map(text -> new Property<>(text.valueProperty().get(), Confidence.USER)).collect(Collectors.toList()));
            }
        });
    }
}
