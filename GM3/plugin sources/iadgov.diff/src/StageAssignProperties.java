...
public class StageAssignProperties extends AbstractStage<Session> {
    ...
    public final static Confidence CONFIDENCE = Confidence.USER;

    public static class EdgeProperties implements IHasLogicalConnectionProperties {
        private final Session.LogicalAddressPair endpoints;
        private final Map<String, Collection<Property<?>>> properties;

        public EdgeProperties(final Session.LogicalAddressPair endpoints, final String property, final Serializable value) {
            this.endpoints = endpoints;

            this.properties = Collections.singletonMap(property, Collections.singleton(new Property<>(value, CONFIDENCE)));
        }
    ...
    public StageAssignProperties(final RuntimeConfiguration config, final Session session) {
        super(config, session, ILogicalPacketMetadata.class, Session.LogicalAddressPair.class, HardwareAddress.class, LogicalAddressMapping.class);
    ...
    public Object process(final Object o) {
        if(o instanceof Session.LogicalAddressPair) {
            if (this.options.tagEdges) {
                return new EdgeProperties((Session.LogicalAddressPair) o, this.options.property, this.options.value);
            }
        } else if(o instanceof ILogicalPacketMetadata) {
            //HACK: These should be reported as AddressPairs as well
            if(this.options.tagEdges) {
                return new EdgeProperties(new Session.LogicalAddressPair(((ILogicalPacketMetadata)o).getSourceAddress(), ((ILogicalPacketMetadata)o).getDestAddress()), this.options.property, this.options.value);
            }
