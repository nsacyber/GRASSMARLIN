...
public class VisualAbstractStage extends VBox implements IDraggable {
    ...
    public VisualAbstractStage(final AbstractStage<?> stage, final StatePollingScheduler scheduler) {
        this.stage = stage;
    ...
    private void initComponents(final StatePollingScheduler scheduler) {
        final LocalDate today = LocalDate.now();
        final boolean showSpm = today.getMonth() == Month.MARCH && today.getDayOfMonth() == 31;
        final QueueStateDoubleBinding bindingQueue = new QueueStateDoubleBinding(this.stage.getQueue(), scheduler);

        this.setBackground(new Background(new BackgroundFill(VisualTab.VISUAL_ELEMENT_CLIENT_FILL, VisualTab.VISUAL_ELEMENT_CORNER_RADII, null)));
        ...
        final HBox containerTitle = new HBox(VisualTab.VISUAL_ELEMENT_PADDING);
        containerTitle.setBackground(new Background(new BackgroundFill(VisualTab.VISUAL_ELEMENT_TITLE_FILL, VisualTab.VISUAL_ELEMENT_CORNER_RADII_TOP_ONLY, null)));
        final HBox containerContent = new HBox(VisualTab.VISUAL_ELEMENT_PADDING * 2.0);
        final GridPane contentStats = new GridPane();
        contentStats.setOpaqueInsets(new Insets(VisualTab.VISUAL_ELEMENT_PADDING));
        final VBox containerOutputs = new VBox(VisualTab.VISUAL_ELEMENT_PADDING);
        ...
        title.setFill(VisualTab.VISUAL_ELEMENT_TITLE_TEXT_FILL);
        ...
        labelAcceptDelay.textProperty().bind(new FxStringProperty(new AtomicLongBinding(this.stage.getAcceptDelay(), scheduler).asString()));
        ...
        textInputsAccepted.textProperty().bind(new FxStringProperty(new AtomicLongBinding(this.stage.getInputsAccepted(), scheduler).asString()));
        ...
        textInputsProcessed.textProperty().bind(new FxStringProperty(new AtomicLongBinding(this.stage.getInputsProcessed(), scheduler).asString()));
        ...
        textOutputsProcessed.textProperty().bind(new FxStringProperty(new AtomicLongBinding(this.stage.getOutputsProcessed(), scheduler).asString()));
        ...
        textOutputsDropped.textProperty().bind(new FxStringProperty(new AtomicLongBinding(this.stage.getOutputsDropped(), scheduler).asString()));
        ...
        for(final String output : this.stage.getOutputs().stream().sorted().collect(Collectors.toList()) ) {
            final HBox containerOutput = new HBox(VisualTab.VISUAL_ELEMENT_PADDING);
