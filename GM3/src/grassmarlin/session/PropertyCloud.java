    ...
    protected void addPropertyToCache(final String name, final Serializable value, final int confidence) {
        synchronized (propertiesComputed) {
            ...
            if (values == null) {
                ...
                Integer confidenceExisting = values.stream().filter(property -> property.getValue().equals(value)).map(Property::getConfidence).sorted().findFirst().orElse(null);
            ...
            synchronized(propertiesProvided) {
                final List<Property<?>> propertiesForLine = Stream.concat(
                    ...
                    .map(entry -> entry.getValue().stream()
                            .sorted((p1, p2) -> p1.getConfidence() - p2.getConfidence())    // Sort by ascending confidence (highest confidence first, which is lowest numeric value)
                            .findFirst()
                            .get())
