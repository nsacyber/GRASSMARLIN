...
public class Pipeline implements AutoCloseable {
    ...
    public static Pipeline buildPipelineFromTemplate(final RuntimeConfiguration config, final Session session, PipelineTemplate source) {
        if(source == null) {
            source = config.getDefaultPipelineTemplate();
        }
        ...
        for (Map.Entry<String, List<PipelineStageConnection>> entry : connectionMap.entrySet()) {
            if (entry.getValue().size() > 1) {
                ...
                result.addMulticastLink(config, stageSource, nameOutput, destinations.toArray(new AbstractStage[destinations.size()]));
    ...
    protected void addMulticastLink(final RuntimeConfiguration config, final AbstractStage<Session> source, final String nameOutput, final AbstractStage<Session>... destinations) {
        if (stages.contains(source) && stages.containsAll(Arrays.asList(destinations))) {
