...
public abstract class Chart<T, TX extends Comparable<TX>, TY extends Comparable<TY>> extends Canvas {
    ...
    protected double widthLine = 1.0;
    ...
    protected void paint(final GraphicsContext gc) {
        ...
        try {
            rectChart = new Rectangle2D(sizeAxisLabel.getWidth() + this.pxTickLength, 0.0, getWidth() - sizeAxisLabel.getWidth() - this.pxTickLength, getHeight() - sizeAxisLabel.getHeight() - pxTickLength);
        ...
        for(final Map.Entry<Series, LinkedList<T>> entry : data.entrySet()) {
            ...
            gc.setLineWidth(this.widthLine);
            ...
            for(final T value : entry.getValue()) {
                ...
                gc.strokeOval(ptNew.getX() - this.pointRadius, ptNew.getY() - pointRadius, pointRadius * 2.0, pointRadius * 2.0);
                ptPrev = ptNew;
            }
        }
        ...
        gc.clearRect(0.0, 0.0, sizeAxisLabel.getWidth() + this.pxTickLength, this.getHeight());
        gc.clearRect(0.0, this.getHeight() - sizeAxisLabel.getHeight() - this.pxTickLength, this.getWidth(), sizeAxisLabel.getHeight() + this.pxTickLength);
        //Draw the axes
        gc.setStroke(Color.BLACK);
        gc.setLineWidth(0.5);
        gc.strokeLine(rectChart.getMinX(), rectChart.getMinY(), rectChart.getMinX(), rectChart.getMaxY());
        gc.strokeLine(rectChart.getMinX(), rectChart.getMaxY(), rectChart.getMaxX(), rectChart.getMaxY());

        gc.setFont(this.fontLabels);
        ...
        for(final TX tickX : ticksX) {
            ...
            pxLast = pxX;
            gc.strokeLine(pxX, rectChart.getMaxY(), pxX, rectChart.getMaxY() + this.pxTickLength);
            ...
            gc.rotate(-30.0);
            gc.fillText(textLabel, 0.0, 0.0);
            gc.restore();
        }
        for(final TY tickY : ticksY) {
            final double pxY = chartYFromDataY(axisY, tickY);
            gc.strokeLine(rectChart.getMinX() - this.pxTickLength, pxY, rectChart.getMinX(), pxY);
            final String textLabel = formatY(tickY);
            gc.strokeText(textLabel, 0.0, pxY + TextMeasurer.measureText(textLabel, this.fontLabels).getHeight());
        }
    }
