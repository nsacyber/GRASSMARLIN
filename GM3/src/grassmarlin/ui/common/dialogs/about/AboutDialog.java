...
public class AboutDialog extends Dialog<ButtonType> {
    ...
    private void initComponents() {
        ...
        this.config.setIcons(this);

        final HBox layout = new HBox(2.0);
        final ImageView imageIconLarge = new ImageView(new Image(AboutDialog.class.getResourceAsStream("/resources/images/grassmarlin_302.png")));
        imageIconLarge.setPreserveRatio(true);
        imageIconLarge.setFitWidth(302.0);
        imageIconLarge.setFitHeight(302.0);

        final GridPane paneDetails = new GridPane();
        final Label textTitle = new Label(Version.APPLICATION_TITLE);
        textTitle.setFont(Font.font("Comic Sans MS", 18.0));   //What are the odds nobody will catch this (and I will forget it) before release?
