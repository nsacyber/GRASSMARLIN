...
public class PreferenceDialog<TPrefs extends Cloneable> extends Dialog {
    ...
    public void setPreferences(final TPrefs preferencesNew) {
        ...
        */
        ...
        for(final java.lang.reflect.Field field : fields) {
            ...
            final String name = details.accessorName() != "" ? details.accessorName() : field.getName().replaceAll("^.", String.valueOf(Character.toUpperCase(field.getName().charAt(0))));
            ...
            try {
                if (field.getType() == Long.class || field.getType() == Integer.class || field.getType() == Double.class) {
                    ...
                    if (!hasSetter) {
                        ...
                        control.selectedProperty().addListener((observable, oldValue, newValue) -> {
                            try {
                                //We don't support boolean nulls.  There is just no UI that works for it--A ComboBox with True/False/FILE_NOT_FOUND is the best bet, and is generally accepted to be awful.
                                clazz.getMethod("set" + name, Boolean.class).invoke(this.preferencesOut, newValue);
                            } catch (NoSuchMethodException | IllegalAccessException | InvocationTargetException ex) {
                                PreferenceDialog.this.hasError.set(true);
                            }
    ...
    public TPrefs getPreferences() {
        if(hasError.getAndSet(false)) {
            Logger.log(Logger.Severity.WARNING, "There were errors computing the new preferene values; the resulting avlues may not be correct.");
        }
