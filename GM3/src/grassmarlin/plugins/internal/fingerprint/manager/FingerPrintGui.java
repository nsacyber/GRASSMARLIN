...
import grassmarlin.plugins.internal.fingerprint.FPDocument;
import grassmarlin.plugins.internal.fingerprint.Plugin;
import grassmarlin.plugins.internal.fingerprint.manager.editorPanes.FilterEditPane;
import grassmarlin.plugins.internal.fingerprint.manager.editorPanes.FilterGroupEditPane;
import grassmarlin.plugins.internal.fingerprint.manager.editorPanes.FingerprintInfoPane;
import grassmarlin.plugins.internal.fingerprint.manager.editorPanes.PayloadEditorPane;
import grassmarlin.plugins.internal.fingerprint.manager.filters.Filter;
import grassmarlin.plugins.internal.fingerprint.manager.tree.FPItem;
import grassmarlin.plugins.internal.fingerprint.manager.tree.FilterGroupItem;
...
public class FingerPrintGui extends Application {
    ...
    public boolean saveFingerprintWODialog(String fpName, Path loadPath) {
        boolean saved;
        if (!(loadPath == null || loadPath.toAbsolutePath().startsWith(this.plugin.getDefaultFingerprintDir()))) {
        ...
        if (!canceled) {
            for (FingerprintState state : unsavedStates) {
                ...
                if (state.pathProperty().get() != null) {
                    try {
                        this.document.load(state.pathProperty().get());
