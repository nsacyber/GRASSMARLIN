...
public class RuntimeConfiguration {
    ...
    public enum PersistableFields implements IPersistedValue {
        ...
        private PersistableFields(final String key, final IDefaultValue fnDefault) {
            this.key = key;
        ...
        for(int idxArg = 0; idxArg < args.length; idxArg++) {
            ...
            switch(arg) {
                ...
                case "-P":
                    ...
                    final String[] parameters = args[++idxArg].split(":", 2);
                    if(parameters.length == 1) {
                        pluginArguments.put(parameters[0], args[++idxArg]);
                    } else {
                        pluginArguments.put(parameters[0], parameters[1]);
                    }
            }
        }
                ...
                try {
                    ...
                    while (!pluginsToSort.isEmpty()) {
                        ...
                        for (final Class<? extends IPlugin> classPlugin : pluginsToSort) {
                            if (classPlugin.isAnnotationPresent(IPlugin.Uses.class)) {
                                //If any plugins this plugin requires are still in the to-move list, skip it.
                                if (!Arrays.stream(classPlugin.getAnnotation(IPlugin.Uses.class).value()).map(name -> pluginClasses.get(name)).anyMatch(clazz -> pluginsToSort.contains(clazz))) {
                    ...
                    for (final Class<? extends IPlugin> classPlugin : pluginsToLoad) {
                        try {
                            ...
                            Logger.log(Logger.Severity.ERROR, "Unable to load Plugin '%s': %s", pluginNameFor(classPlugin), ex.getMessage());
                        }
                    }
    ...
    public final IPlugin.ImportProcessorWrapper importerForFile(final Path pathFile) {
        ...
        for(IPlugin.HasImportProcessors plugin : enumeratePlugins(IPlugin.HasImportProcessors.class)) {
            for(IPlugin.ImportProcessorWrapper wrapper : plugin.getImportProcessors()) {
                for(String extension : wrapper.getExtensions()) {
    ...
    public PipelineTemplate getDefaultPipelineTemplate() {
        try {
            return PipelineTemplate.loadTemplate(this, RuntimeConfiguration.class.getResourceAsStream("/resources/default.pte"));
        } catch(Exception ex) {
