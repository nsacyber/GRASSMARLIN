...
public class FingerPrintGui extends Application {
    ...
    private void checkDirtyOnClosing(WindowEvent event) {
        ...
        for (FingerprintState state : this.document.getFingerprints()) {
            if (state.getDirtyProperty()) {
                Optional<ButtonType> choice = this.getSaveOnCloseDialog(state.getFingerprint().getHeader().getName(),
                        state.getPathProperty()).showAndWait();

                if (choice.isPresent()) {
                    switch (choice.get().getButtonData()) {
                        case YES:
                            boolean saved = this.saveFingerprintWODialog(state.getFingerprint().getHeader().getName(), state.getPathProperty());
                            if (!saved) {
                                event.consume();
                                break mainLoop;
                            }
    ...
    public void enableAll(ActionEvent event) {
        this.document.getFingerprints().forEach(state -> state.setEnabledProperty(true));
    }

    public void disableAll(ActionEvent event) {
        this.document.getFingerprints().forEach(state -> state.setEnabledProperty(false));
    }
