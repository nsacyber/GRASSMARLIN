    ...
    private void initComponents() {
        ...
        optSource.setOnAction(event -> {
            nodesFiltered.setPredicate(logicalNode ->
                            network.get().getEdgesInvolving(logicalNode).stream()
                                    .anyMatch(edge ->
                                            (edge.getSource().equals(logicalNode) && edge.getDetailsToDestination().getCntBytes() > 0)
                                            ||
                                            (edge.getDestination().equals(logicalNode) && edge.getDetailsToSource().getCntBytes() > 0)
                                    )
            );
        });
        optDestination.setOnAction(event -> {
            nodesFiltered.setPredicate(logicalNode ->
                            network.get().getEdgesInvolving(logicalNode).stream()
                                    .anyMatch(edge ->
                                            (edge.getSource().equals(logicalNode) && edge.getDetailsToSource().getCntBytes() > 0)
                                            ||
                                            (edge.getDestination().equals(logicalNode) && edge.getDetailsToDestination().getCntBytes() > 0)
                                    )
            );
        });
