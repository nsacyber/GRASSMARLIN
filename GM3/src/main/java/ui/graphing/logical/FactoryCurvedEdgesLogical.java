...
public class FactoryCurvedEdgesLogical extends FactoryCurvedEdges<LogicalNode, LogicalEdge> {
    ...
    public Edge uiFor(LogicalEdge edge, Cell source, Cell destination) {
        ...
        FxStringProperty p = new FxStringProperty();
        p.setValue(edge.getEdgeDetailsProperty());
        result.setDetails(p);

        //Weight is log10(bytes sent+bytes received) / 100.
        result.weightProperty().bind(defaultWeight);
        ...
        /*
        edge.getDetailsToDestination().getBytes().addListener((observable, oldValue, newValue) -> {
            Platform.runLater(() -> {
                source.HighlightForChange();
                destination.HighlightForChange();
            });
        });
        edge.getDetailsToSource().getBytes().addListener((observable, oldValue, newValue) -> {
            Platform.runLater(() -> {
                source.HighlightForChange();
                destination.HighlightForChange();
            });
