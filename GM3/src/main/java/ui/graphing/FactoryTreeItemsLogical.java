    ...
    public GraphTreeItem.GraphTreeEdgeItem<LogicalNode, LogicalEdge> TreeNodeForEdge(LogicalEdge edge, LogicalNode parent) {
        ...
        if(nodeNew.requiresInitialization()) {
            ...
            if (edge.getSource().equals(parent)) {
                String newTitleProperty = nodeNew.getNode().titleProperty().get();
                newTitleProperty += ": " + nodeNew.getEdge().getDetailsToSource().getCntBytes();
                newTitleProperty += " bytes received / " + nodeNew.getEdge().getDetailsToDestination().getCntBytes();
                newTitleProperty += " bytes sent";
                nodeNew.valueProperty().bind(new SimpleStringProperty(newTitleProperty));
                /*
                ...
                */
                ...
                String newTitleProperty = nodeNew.getNode().titleProperty().get();
                newTitleProperty += ": " + nodeNew.getEdge().getDetailsToDestination().getCntBytes();
                newTitleProperty += " bytes received / ";
                newTitleProperty += nodeNew.getEdge().getDetailsToSource().getCntBytes();
                newTitleProperty += " bytes sent";
                nodeNew.valueProperty().bind(new SimpleStringProperty(newTitleProperty));
                /*
