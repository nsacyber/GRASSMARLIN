...
public class TcpPacket extends PacketDataWrapper implements ITcpPacketMetadata, ITcpPacketData {
    ...
    public TcpPacket(final ILogicalPacketData packet, final int idxTcpHeaderStart, final int cbTcpHeader, final LogicalAddressMapping addrSource, final LogicalAddressMapping addrDest) {
        super(packet);
    ...
    @Override
    public int getFlags() {
        return packet.getContents().getShort(packet.getContents().position() + offsetTcpHeader + 12) & 0x01FF;
    }
