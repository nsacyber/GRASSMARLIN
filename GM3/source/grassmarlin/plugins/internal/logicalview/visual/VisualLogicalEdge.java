...
public class VisualLogicalEdge extends Group {
    ...
    //The BoundEdgeProperties include preferred color/weighting/opacity
    private BoundEdgeProperties boundProperties;

    public VisualLogicalEdge(final LogicalVisualization visualization, final GraphLogicalEdge edge, final VisualLogicalVertex vertSource, final VisualLogicalVertex vertDest, final ObservableList<EdgeStyleRule> rules, final BooleanExpression useRules) {
        this.visualization = visualization;
        this.edge = edge;
        this.vertSource = vertSource;
        this.vertDest = vertDest;

        this.boundProperties = new BoundEdgeProperties(this, rules, useRules);
    ...
    protected void layoutChildren() {
        ...
        if(isEdgeStructureChanging) {
            ...
            for(final EndpointPair pair : endpointsNew) {
                final DirectedCurve curve = new DirectedCurve(this.visualization, pair.ptStart, pair.ptEnd, this.boundProperties);
            ...
            for(Map.Entry<EndpointPair, DirectedCurve> pair : this.curveFromEndpoint.entrySet()) {
                ...
                pair.getValue().strokeWidthProperty().bind(
                        new When(this.visualization.useWeightedEdgesProperty())
    ...
    private InvalidationListener handlerPacketListsChanged = this::handlePacketListsChanged;
    private void handlePacketListsChanged(final Observable obs) {
        isEdgeStructureChanging = true;
        this.requestLayout();
    }
