...
import grassmarlin.ui.common.PaintBindingForColor;
...
public class DirectedCurve extends Group {
    protected static final double LENGTH_ARROW = 5.6;
    protected static final double HEIGHT_ARROW = 2.0;
    ...
    private final static double[] ptsArrow = new double[] {0.0, -HEIGHT_ARROW, LENGTH_ARROW, 0.0, 0.0, HEIGHT_ARROW};
    ...
    public DirectedCurve(final LogicalVisualization visualization, final VisualLogicalVertex.ContentRow rowSource, final VisualLogicalVertex.ContentRow rowDestination, final BoundEdgeProperties styleModifiers) {
        this.line = new Line();
        ...
        final ControlPointAngleBinding degAtSource = new ControlPointAngleBinding(new When(visualization.useCurvedEdgesProperty()).then(this.controlX1Property).otherwise(this.endXProperty), new When(visualization.useCurvedEdgesProperty()).then(this.controlY1Property).otherwise(this.endYProperty), this.startXProperty, this.startYProperty);
        final ControlPointAngleBinding degAtDestination = new ControlPointAngleBinding(new When(visualization.useCurvedEdgesProperty()).then(this.controlX2Property).otherwise(this.startXProperty), new When(visualization.useCurvedEdgesProperty()).then(this.controlY2Property).otherwise(this.startYProperty), this.endXProperty, this.endYProperty);
        ...
        final NumberBinding startX = new When(this.arrowAtSource.visibleProperty()).then(new AngledOffsetXBinding(this.startXProperty, degAtSource, this.scaleEndpoints.xProperty().multiply(-LENGTH_ARROW))).otherwise(this.startXProperty);
        final NumberBinding startY = new When(this.arrowAtSource.visibleProperty()).then(new AngledOffsetYBinding(this.startYProperty, degAtSource, this.scaleEndpoints.yProperty().multiply(-LENGTH_ARROW))).otherwise(this.startYProperty);
        final NumberBinding endX = new When(this.arrowAtDestination.visibleProperty()).then(new AngledOffsetXBinding(this.endXProperty, degAtDestination, this.scaleEndpoints.xProperty().multiply(-LENGTH_ARROW))).otherwise(this.endXProperty);
        final NumberBinding endY = new When(this.arrowAtDestination.visibleProperty()).then(new AngledOffsetYBinding(this.endYProperty, degAtDestination, this.scaleEndpoints.yProperty().multiply(-LENGTH_ARROW))).otherwise(this.endYProperty);
        ...
        this.line.strokeProperty().bind(
                new When(styleModifiers.colorProperty().isNotNull())
                        .then(new PaintBindingForColor(styleModifiers.colorProperty()))
                        .otherwise(new BoundLinearGradient(startX, startY, endX, endY, rowSource.backgroundColorBinding(), rowDestination.backgroundColorBinding()))
        );

        final ObjectExpression<Color> colorAtSource = new When(styleModifiers.colorProperty().isNotNull()).then(
                styleModifiers.colorProperty()
        ).otherwise(
                rowSource.backgroundColorBinding()
        );
        final ObjectExpression<Color> colorAtDestination = new When(styleModifiers.colorProperty().isNotNull()).then(
                styleModifiers.colorProperty()
        ).otherwise(
        ...
        this.line.strokeWidthProperty().bind(this.strokeWidthProperty.multiply(styleModifiers.weightProperty()));

        this.opacityProperty().bind(styleModifiers.opacityProperty());

        this.line.visibleProperty().bind(visualization.useCurvedEdgesProperty().not());
        this.curve.visibleProperty().bind(visualization.useCurvedEdgesProperty());
        ...
        this.arrowAtSource.fillProperty().bind(colorAtSource);
        this.arrowAtDestination.fillProperty().bind(colorAtDestination);
