...
public class StageRecordLogicalPacketData extends AbstractStage<Session> {
    ...
    protected final static int NUM_THREADS = 1; //Increasing this tends to not offer any performance gains--the bottleneck just shifts elsewhere.  Depending on how the pipeline is built, though, increasing this might have merit.
    ...
    public StageRecordLogicalPacketData(final RuntimeConfiguration config, final Session session) {
        ...
        super(config, session, new ArrayBlockingQueue<>(1000), ILogicalPacketMetadata.class);

        this.plugin = ((AggregatePlugin)config.pluginFor(StageRecordLogicalPacketData.class)).getMember(Plugin.class);
    ...
    public Object process(final Object obj) {
        //We only accept ILogicalPacketMetadata
        final ILogicalPacketMetadata packet = (ILogicalPacketMetadata) obj;
        ...
        this.plugin.recordPacketForSession(this.getContainer(), packet);
