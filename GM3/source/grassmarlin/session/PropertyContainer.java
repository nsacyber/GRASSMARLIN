...
public class PropertyContainer implements XmlSerializable {
    ...
    public Map<String, Set<Property<?>>> getProperties() {
        synchronized(computedProperties) {
            return new HashMap<>(this.computedProperties);
        }
    }
    public Map<String, Map<String, Set<Property<?>>>> getProvidedProperties() {
    ...
    public boolean hasPropertyValue(final String nameProperty, final Serializable value) {
        synchronized(this.computedProperties) {
            final Set<Property<?>> values = (this.computedProperties.get(nameProperty));
            return values != null && values.stream().anyMatch(property -> property.getValue().equals(value));
        }
    }
        ...
        synchronized(computedProperties) {
            ...
            if(values == null) {
                ...
                while(iterator.hasNext()) {
                    ...
                    final int c = prop.getConfidence().compareTo(confidence);
                    if(c < 0) {
                        iterator.remove();
    ...
    protected void rebuildCacheLine(final String name) {
        ...
        synchronized(ancestors) {
            synchronized(providedProperties) {
                final Confidence confidenceTarget = Confidence.max(
                        providedProperties.values().stream()
                            .map(map -> map.get(name))
                            .filter(set -> set != null)
                            .flatMap(set -> set.stream())
                            .map(Property::getConfidence)
                            .max(Confidence.COMPARATOR).orElse(Confidence.NONE),
                        ancestors.stream()
                ...
                if(confidenceTarget == Confidence.NONE) {
