...
import grassmarlin.common.fxobservables.QueueStateBinding;
...
public abstract class AbstractStage<TContainer> implements Consumer<Object>, AutoCloseable {
    ...
    private final QueueStateBinding queueWrapper;

    //Stat trackers
    private final LongAccumulator cntInputsAccepted;
    private final LongAccumulator cntInputsProcessed;
    private final LongAccumulator cntOutputsProcessed;
    private final LongAccumulator cntOutputsDropped;

    private final LongAccumulator msAcceptDelay;
    ...
    protected AbstractStage(final RuntimeConfiguration config, final TContainer container, final BlockingQueue<Object> backlog, final Collection<Class<?>> inputs) {
        ...
        this.queueWrapper = new QueueStateBinding(config.getUiEventProvider(), backlog);
        ...
        this.cntInputsAccepted = new LongAccumulator(config, 0);
        this.cntInputsProcessed = new LongAccumulator(config, 0);
        this.cntOutputsProcessed = new LongAccumulator(config, 0);
        this.cntOutputsDropped = new LongAccumulator(config, 0);

        this.msAcceptDelay = new LongAccumulator(config, 0);
    ...
    protected void processOutput(final Object output) {
        if(output == null) {
            ...
            this.cntOutputsProcessed.increment(1);
            ...
            if(!processedOutput) {
                this.cntOutputsDropped.increment(1);
            }
        }
    }

    public void accept(Object o) {
        this.cntInputsAccepted.increment(1);
        try {
            for(Class<?> clazz : typesAccepted) {
                if(clazz.isInstance(o)) {
                    while(!queueWrapper.offer(o)) {
                        this.msAcceptDelay.increment(1);
                        Thread.sleep(1);
                    }
        ...
        while(!isTerminating) {
            try {
                ...
                try {
                    ...
                    Logger.log(Logger.Severity.ERROR, "Error while processing Object [%s] in Stage [%s]: %s", item, this.getClass().getName(), ex.getMessage());
                }
                this.cntInputsProcessed.increment(1);
            } catch(InterruptedException ex) {
                if(isTerminating) {
                    //Ignore it.
                } else {
    ...
    public QueueStateBinding getQueue() {
        return this.queueWrapper;
    }

    public LongAccumulator getInputsAccepted() {
        return cntInputsAccepted;
    }
    public LongAccumulator getInputsProcessed() {
        return this.cntInputsProcessed;
    }
    public LongAccumulator getOutputsProcessed() {
        return this.cntOutputsProcessed;
    }
    public LongAccumulator getOutputsDropped() {
        return this.cntOutputsDropped;
    }

    public LongAccumulator getAcceptDelay() {
