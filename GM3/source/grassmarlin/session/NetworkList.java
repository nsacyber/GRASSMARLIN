...
public class NetworkList implements XmlSerializable {
    ...
    private class NetworkListThreadManagedState extends ThreadManagedState {
        ...
        public void validate() {
            ...
            final HashMap<Confidence, List<Network>> intermediateMapping = new HashMap<>();
            ...
            synchronized(reportedNetworks) {
                for (Set<Network> set : reportedNetworks.values()) {
                    ...
                    List<Confidence> confidences = set.stream().map(network -> network.getConfidence()).distinct().sorted().collect(Collectors.toList());
            ...
            for(Confidence confidence : intermediateMapping.keySet().stream().sorted().collect(Collectors.toList())) {
    ...
    public NetworkList(final Event.IAsyncExecutionProvider eventExecutor) {
        this.computeExecutor = Event.createThreadQueueProvider();
