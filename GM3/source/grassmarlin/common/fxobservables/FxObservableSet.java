...
public class FxObservableSet<E> implements ObservableSet<E> {
    ...
    public FxObservableSet(final Event.IAsyncExecutionProvider ui) {
        this(ui, new ConcurrentSkipListSet<>());
    }
    public FxObservableSet(final Event.IAsyncExecutionProvider ui, final Set<E> set) {
        this.uiProvider = ui;
        this.backingSet = set;
    }
    ...
    private void callObservers(SetChangeListener.Change<E> change) {
        try {
            if(!uiProvider.isExecutionThread()) {
                uiProvider.runLater(() -> SetListenerHelper.fireValueChangedEvent(listenerHelper, change));
            } else {
