...
public class ZoomableScrollPane extends Pane implements IAltClickable, ICanHasContextMenu, XmlSerializable {
    ...
    public boolean addChild(final Node child) {
        return this.zoomGroup.getChildren().add(child);
    }

    public boolean addChild(final Node child, final String layer) {
        return this.layers.get(layer).getChildren().add(child);
    }
    ...
    private static IDraggable findTopMostDraggable(final Parent root, final Point2D ptUnder) {
        //The last element is the top-most visually.
        final LinkedList<Node> children = new LinkedList<>(root.getChildrenUnmodifiable());
        Collections.reverse(children);
        for(final Node child : children) {
            final Point2D ptInChild = child.parentToLocal(ptUnder);
            //If it is invisible, we skip it.
            //If it doesn't contain the point, we skip it.
            if(!child.isVisible() || !child.contains(ptInChild)) {
                continue;
            }

            if(child instanceof IDraggable && ((IDraggable)child).isDraggable()) {
                return (IDraggable)child;
            }
            if(child instanceof Parent) {

                final IDraggable result = findTopMostDraggable((Parent)child, ptInChild);
                if(result != null) {
                    return result;
                }
            }
    ...
    private void handleMousePressed(MouseEvent event) {
        if (event.getButton() == MouseButton.PRIMARY) {
            event.consume(); //It is a little-known fact that events taste like delicious, imaginary, deceitful cake.

            this.dragTarget = findTopMostDraggable(this.zoomGroup, this.zoomGroup.screenToLocal(event.getScreenX(), event.getScreenY()));
            if(this.dragTarget != null) {
                final Point2D worldTarget;
                final Point2D sceneTarget = new Point2D(event.getSceneX(), event.getSceneY());
                if(this.dragTarget instanceof Node) {
                    final Node node = (Node)this.dragTarget;
                    Point2D ptTemp;
                    try {
                        ptTemp = node.getParent().getLocalToSceneTransform().inverseTransform(sceneTarget);
                ...
                this.dragTarget.startDrag(worldTarget);
            } else {
            ...
            if(this.dragTarget == null) {
                ...
                if(this.dragTarget instanceof Node) {
                    ...
                    try {
                        ...
                        if(parent != null) {
                            ptTemp = parent.getLocalToSceneTransform().inverseTransform(sceneTarget);
                        } else {
                            ptTemp = sceneTarget;
                        }
                    } catch (NonInvertibleTransformException ex) {
                ...
                this.dragTarget.dragTo(worldTarget);
            }
        }
    }
    ...
    private void handleMouseWheelZoom(ScrollEvent event) {
        ...
        if (delta < 0) {
            this.zoomTo(this.scaleLevel - 1, ptEvent);
        } else if (delta > 0) {
            this.zoomTo(this.scaleLevel + 1, ptEvent);
        }
        event.consume();
                ...
                if (draggable instanceof Node) {
                    ...
                    if (bounds.getWidth() > 0 && bounds.getHeight() > 0) {
                        if (!Double.isNaN(node.getTranslateX())) {
                            ptTopLeft.x = Double.min(ptTopLeft.x, node.getTranslateX() + node.getLayoutX());
                            if (!Double.isNaN(bounds.getWidth())) {
                                ptBottomRight.x = Double.max(ptBottomRight.x, node.getTranslateX() + node.getLayoutX() + bounds.getWidth());
                            }
                        }
                        if (!Double.isNaN(node.getTranslateY())) {
                            ptTopLeft.y = Double.min(ptTopLeft.y, node.getTranslateY() + node.getLayoutY());
                            if (!Double.isNaN(bounds.getHeight())) {
                                ptBottomRight.y = Double.max(ptBottomRight.y, node.getTranslateY() + node.getLayoutY() + bounds.getHeight());
                            }
                        }
                    }
                }
    ...
    public void zoomToFit() {
        ...
        if(boundsNodes == null || boundsNodes.getWidth() == 0.0 || boundsNodes.getHeight() == 0.0) {
            ...
            if(this.getWidth() == 0.0) {
                ...
                scaleX = getWidth() / boundsNodes.getWidth();
            }
            if(this.getHeight() == 0.0) {
                scaleY = 1.0;
            } else {
                scaleY = getHeight() / boundsNodes.getHeight();
            }
