...
public class SavingTask extends AsyncUiTask<SavingTask> {
    ...
    public SavingTask(final RuntimeConfiguration config, final SingleDocumentState state, final AsyncUiTask.TaskCallback<SavingTask> onSuccess, final AsyncUiTask.TaskCallback<SavingTask> onFailure) {
        this(config, state, Paths.get(state.currentSessionPathProperty().get()), onSuccess, onFailure);
    }

    public SavingTask(final RuntimeConfiguration config, final SingleDocumentState state, final Path path, final AsyncUiTask.TaskCallback<SavingTask> onSuccess, final AsyncUiTask.TaskCallback<SavingTask> onFailure) {
    ...
    protected void handleSuccess(SavingTask self) {
        Platform.runLater(() -> this.state.saved(SavingTask.this.path.toAbsolutePath().toString()));
