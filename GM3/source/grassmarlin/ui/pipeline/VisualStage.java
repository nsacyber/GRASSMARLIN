...
import grassmarlin.ui.pipeline.edit.ActionDrag;
...
public class VisualStage extends VBox implements IDraggable, IAltClickable, ICanHasContextMenu, Linkable {
    ...
    private PanePipelineEditor container;
    ...
    public VisualStage(PipelineTemplate template, IPlugin.PipelineStage stage, PanePipelineEditor container, RuntimeConfiguration config) {
    ...
    public void dragTo(final Point2D ptDestination) {
        this.container.doEdit(new ActionDrag(this, ptDestination));
    }
    ...
    private HBox createTitleRow() {
        ...
        if (configurable) {
            ...
            configButton.setOnMousePressed(event -> {
                this.container.doEdit(new IActionUndoable() {
                    Serializable oldConfig = template.getStageConfiguration(VisualStage.this.stage.getName());

                    @Override
                    public boolean undoAction() {
                        template.setConfiguration(VisualStage.this.stage.getName(), oldConfig);
                        //TODO: Give some indication of action.
